...
import { Plus, Search, User, Edit3 } from 'lucide-react';
import {
...
export default function CustomersPage() {
  ...
  const toggleCustomer = async (customer: Customer) => {
    try {
      ...
      setMessage({ type: 'success', text: nextStatus === 'active' ? 'Customer activated.' : 'Customer deactivated.' });
      await load();
    } catch (err: any) {
      setMessage({ type: 'error', text: err?.response?.data?.message || 'Could not update customer.' });
    }
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setEditing(null);
      setForm({ name: '', phone: '', email: '', address: '', credit_limit: '', });
      setShowForm(false);
      load();
    } catch (err: unknown) {
      const msg =
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          onClick={() => { setEditing(null); setForm({ name: '', phone: '', email: '', address: '', credit_limit: '' }); setShowForm(true); }}
      ...
      {loading ? (
        <div className="py-16 text-center text-slate-500">Loading customers…</div>
      ) : customers.length === 0 ? (
        <div className="rounded-xl bg-white py-16 text-center text-slate-500 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-400">Add your first customer to keep customer records.</p>
        </div>
      ) : (
            ...
            {customers.map((c) => {
              ...
              return (
                <li key={c.id} className="flex flex-wrap items-center gap-3 px-4 py-3">
                  <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-teal-50 text-sm font-bold text-teal-700">
                  ...
                  <div className="text-right">
                    {balance > 0 ? (
                      <>
                        ...
                        <div className="text-[10px] text-amber-600">Owes</div>
                      </>
                    ) : (
                      <div className="text-xs text-slate-400">No balance</div>
                    )}
                    <div className="flex flex-wrap gap-1"><button type="button" onClick={() => openEdit(c)} className="rounded-lg bg-slate-100 p-2 text-slate-700" title="Edit"><Edit3 className="h-4 w-4" /></button><button type="button" onClick={() => toggleCustomer(c)} className={`rounded-lg px-2.5 py-1.5 text-xs font-semibold ${c.status === 'active' ? 'bg-red-50 text-red-700' : 'bg-emerald-50 text-emerald-700'}`}>{c.status === 'active' ? 'Deactivate' : 'Activate'}</button></div>
                  </div>
                </li>
              );
            })}
        ...
        <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/40 p-4 sm:items-center">
          ...
          >
            ...
            <div className="mt-5 flex gap-2">
              ...
              >
                {submitting ? 'Saving…' : editing ? 'Save changes' : 'Save customer'}
              </button>
            </div>
          </form>
        </div>
