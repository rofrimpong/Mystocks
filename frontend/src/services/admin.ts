...
export async function getAdminBusinesses(){const {data}=await api.get('/admin/businesses',{params:{per_page:50}}); return data.data as Array<{id:string;name:string;status:string;plan?:string;users_count:number}>;}
export async function updateAdminBusiness(id:string,payload:{status?:string;plan?:string}){const {data}=await api.patch(`/admin/businesses/${id}`,payload); return data.data;}
